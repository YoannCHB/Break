...
el.connected=true;el.response=false;el.json=false;el.correctURL=url;el.send=function(text){this.proto2.send(text);}
...
const auto_verif=function(url){if(url.indexOf('http')+url.indexOf('ws')!=-2){return false;}
...
this.proto1.open('GET',url);this.proto1.onerror=function(e){element.send=false;auto_proto2(element,url);}
this.proto1.onload=function(){element.type=["XMLHttpRequest",'GET',element.responseType];element.connected=true;element.coorectURL=url;element.response=element.proto1.response;element.send=function(text){let req=new XMLHttpRequest();req.open('POST',this.url);req.onerror=function(e){console.warn(e);}
req.send(text);}
try{var jsonGet=JSON.parse(element.proto1.response);}
catch(e){var jsonGet=false;}
if(jsonGet){element.json=j;}
